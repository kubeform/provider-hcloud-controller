...
// IpAssignmentReconciler reconciles a IpAssignment object
type IpAssignmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=floating.hcloud.kubeform.com,resources=ipassignments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=floating.hcloud.kubeform.com,resources=ipassignments/status,verbs=get;update;patch

func (r *IpAssignmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipassignment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpAssignment")
...
func (r *IpAssignmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &floatingv1alpha1.IpAssignment{}); err != nil {
			klog.Error(err, "unable to set up auditor", floatingv1alpha1.IpAssignment{}.APIVersion, floatingv1alpha1.IpAssignment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&floatingv1alpha1.IpAssignment{}).
