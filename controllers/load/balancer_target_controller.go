...
// BalancerTargetReconciler reconciles a BalancerTarget object
type BalancerTargetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.hcloud.kubeform.com,resources=balancertargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.hcloud.kubeform.com,resources=balancertargets/status,verbs=get;update;patch

func (r *BalancerTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancertarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerTarget")
...
func (r *BalancerTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerTarget{}.APIVersion, loadv1alpha1.BalancerTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerTarget{}).
