...
// BalancerNetworkReconciler reconciles a BalancerNetwork object
type BalancerNetworkReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.hcloud.kubeform.com,resources=balancernetworks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.hcloud.kubeform.com,resources=balancernetworks/status,verbs=get;update;patch

func (r *BalancerNetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancernetwork", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerNetwork")
...
func (r *BalancerNetworkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerNetwork{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerNetwork{}.APIVersion, loadv1alpha1.BalancerNetwork{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerNetwork{}).
