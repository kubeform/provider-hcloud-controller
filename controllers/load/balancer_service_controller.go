...
// BalancerServiceReconciler reconciles a BalancerService object
type BalancerServiceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=load.hcloud.kubeform.com,resources=balancerservices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=load.hcloud.kubeform.com,resources=balancerservices/status,verbs=get;update;patch

func (r *BalancerServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("balancerservice", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BalancerService")
...
func (r *BalancerServiceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadv1alpha1.BalancerService{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadv1alpha1.BalancerService{}.APIVersion, loadv1alpha1.BalancerService{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadv1alpha1.BalancerService{}).
